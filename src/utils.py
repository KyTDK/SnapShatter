...
def find_image(image_name, click = True, scale=1.36, click_times=1, roi_height_percentage=100, roi_top_percentage=0, failsafe=True):
    ...
    if max_val > threshold:
        ...
        if attempts>=50:
            ...
            device.shell("am start -n com.snapchat.android/com.snapchat.android.LandingPageActivity")
            time.sleep(5)
            ...
            if previous_image is not None and find_image(previous_image, failsafe=False, click=False):
                print(f"Stuck on {previous_image}, attempting again, {attempts} attempts")
                find_image(previous_image, failsafe=False, click_times=previous_click_times)
            else:
                print(f"{image_name} not found in the screenshot. Certainty of {max_val}, trying again, {attempts} attempts")
            find_image(image_name, click=click, scale=scale, click_times=click_times, roi_height_percentage=roi_height_percentage, roi_top_percentage=roi_top_percentage, failsafe=failsafe)